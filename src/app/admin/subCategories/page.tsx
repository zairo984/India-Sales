...
const Admin = () => {
  ...
  { name: string; description: string; imageUrl: string; id: string }[] | null
>(null);
	...
	async function createSubCategory() {
		try {
			const res = await axios.post(
				`/api/subcategory/`,
				{ name, description, imageUrl: uploadedImageUrl, category: formCategory },
	...
	return (
		<div className="flex min-h-screen bg-gray-100">
			...
			{/* Main Content */}
			<div className={`transition-all duration-300 p-6 w-full ${isSidebarOpen ? "ml-[260px]" : "ml-0"}`}>
							...
							<div key={index} className="w-[280px]">
								<Card className="shadow-md border border-gray-300 bg-white rounded-lg">
									...
									<CardFooter className="p-4 flex justify-between">
										<Button className="bg-red-500 hover:bg-red-600 text-white px-3 py-1.5 rounded-md" onClick={() => deleteSubCategory(item.id)}>
											<FiTrash2 />
										</Button>
			...
			{formOpen && (
				<div className="fixed inset-0 bg-black bg-opacity-40 z-50 flex items-center justify-center">
					<div className="bg-white p-6 rounded-lg shadow-lg w-96">
						...
						<form onSubmit={(e) => { e.preventDefault(); createSubCategory(); }}>
							<input type="text" placeholder="Category Name" value={name} onChange={(e) => setName(e.target.value)} className="w-full border p-2 rounded mb-3" />
							<select className="w-full border p-2 rounded mb-3" onChange={(e) => setFormCategory(e.target.value)}>
								<option value="">-- Select Category --</option>
								{category.map((item, index) => <option key={index} value={item.name}>{item.name}</option>)}
							</select>
							<input type="text" placeholder="Description" value={description} onChange={(e) => setDescription(e.target.value)} className="w-full border p-2 rounded mb-3" />
							<ImageUpload uploadedImageUrl={uploadedImageUrl} setUploadedImageUrl={setUploadedImageUrl} />
							<div className="flex justify-end gap-3 mt-4">
								<Button className="bg-gray-400 hover:bg-gray-500" onClick={() => setFormOpen(false)}>Cancel</Button>
